...
import com.peermountain.pm_livecycle.R;
import com.peermountain.pm_livecycle.model.InfoMessage;
import com.peermountain.pm_livecycle.model.ProgressData;
...
public class BaseUI {
    ...
    private void showNoNetwork() {
        PmDialogUtils.showError(parentView, R.string.pm_msg_no_network);
    }

    private void showError(String msg, NetworkResponse networkResponse) {
        if(networkResponse != null && networkResponse.isErrorHandled) return;

        if (networkResponse != null && networkResponse.errorMsg != 0) {
            PmDialogUtils.showError(parentView, networkResponse.errorMsg);
        } else if (msg != null) {
            PmDialogUtils.showError(parentView, msg);
        } else {
            PmDialogUtils.showError(parentView, R.string.pm_msg_main_server_error);
        }
    }
    ...
    private void showErrorDialog(String msg, NetworkResponse networkResponse) {
        ...
        if (networkResponse != null && networkResponse.errorMsg != 0) {
            PmDialogUtils.showSimpleDialog(context, networkResponse.errorMsg, null);
        } else if (msg != null) {
            PmDialogUtils.showSimpleDialog(context, msg, null);
        } else {
            PmDialogUtils.showSimpleDialog(context, R.string.pm_msg_main_server_error, null);
        }
    }
    ...
    private void showMessage(InfoMessage infoMessage) {
        ...
        if (infoMessage.getMessageResource() != 0) {
            PmDialogUtils.showInfoSnackbar(parentView,infoMessage.getMessageResource());
//            Snackbar.make(parentView, infoMessage.getMessageResource(), Snackbar.LENGTH_LONG).show();
        } else if (infoMessage.getMessage() != null) {
            PmDialogUtils.showInfoSnackbar(parentView,infoMessage.getMessage());
