...
//    }
    ...
    public void sign() throws Exception {
        ...
        if (signature == null) {
            ...
            KeyPair keyPair = SecureHelper.createAndroidKeyStoreAsymmetricKey(InstrumentationRegistry.getTargetContext(), alias);
            Assert.assertNotNull("keyPair null", keyPair);
            Assert.assertNotNull("keyPair.private null", keyPair.getPrivate().getEncoded());
            String pemKey = SecureHelper.toPEM(keyPair.getPrivate());
            LogUtils.e("pemKey", pemKey);
            Assert.assertNotNull("pemKey null", keyPair);
    ...
    @Test
    public void signBouncy() throws Exception {
        String alias = "test2";
        String data = "some information should be hidden";

        KeyPair keyPair = SecureHelper.createAndroidKeyStoreAsymmetricKeyBouncy();
        Assert.assertNotNull("keyPair is null", keyPair);
        Assert.assertNotNull("keyPair.private is null", keyPair.getPrivate().getEncoded());
        String pemKey = SecureHelper.toPEM(keyPair.getPrivate());
        LogUtils.e("pemKey", pemKey);
        Assert.assertNotNull("pemKey is null", keyPair);
        byte[] signature = SecureHelper.sign(data.getBytes(), keyPair.getPrivate());

        if (signature != null) {
            Log.w("sign", new String(signature, "UTF-8"));
        }
        Assert.assertNotNull("signature is null", signature);
        Assert.assertTrue("wrong signature", SecureHelper.verify(data.getBytes(), signature, keyPair.getPublic()));
    }
